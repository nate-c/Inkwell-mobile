...
class InvestmentObject {
  ...
  InvestmentObject(
      this.shares, this.ticker, this.averagePrice, this.currentPrice);
    ...
    if (response.statusCode == 200) {
      ...
      for (int i = 0; i < _investments.length; i++) {
        list = _investments[i]['ticker'] +
            ...
            '\n' +
            'Average Price: \$' +
            _investments[i]['average_price'].toString() +
            '\n' +
            'Current Price: \$' +
            _investments[i]['current_price'].toString();
        totalInvestmentValue = (MyProfileState.investedValue! +
            (_investments
                .map((s) =>
                    s['shares'] * (s['current_price'] - s['average_price']))
